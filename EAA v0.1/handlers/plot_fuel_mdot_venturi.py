# handlers/plot_fuel_mdot_venturi.py
# Calculates Fuel Mass Flow Rate (Mdot) from Venturi measurements
# Equation: ṁ = Cd * Y * A2 * sqrt(2ρ₁(P1-P2) / (1 - (A2/A1)²))
...
def run(app):
    """Open a window to calculate Fuel Mdot from Venturi measurements"""
    ctx = app.ctx
    ...
    VenturiMdotWindow(app, propellant_type="Fuel")
...
class VenturiMdotWindow(tk.Toplevel):
    ...
    def __init__(self, app, propellant_type="Fuel"):
        super().__init__(app)
        self.app = app
        self.ctx = app.ctx
        ...
        self.rho_var = tk.StringVar(value="1000")  # Density in kg/m³
    ...
    def _calculate_and_plot(self):
        ...
        try:
            ...
            # Plot mdot
            raw_line, = ax.plot(time, mdot, label="Raw Data", color="blue", alpha=0.4, linewidth=1)
            smoothed_line, = ax.plot(time, mdot, label="Smoothed Data", color="blue", linewidth=2)
